# ADR-0039 implementation log

This journal preserves the chronological implementation notes, test evidence,
deployment records, and sub-milestone details moved out of the active
[ADR-0039 plan](./adr-0039-shore-remote-access.md). The plan remains authoritative
for architecture, security invariants, sequencing, acceptance gates, and current status.

## Overall historical status
...
- Delivered: Relay skeleton, identity routing, single-host lifecycle, opaque forwarding, and acceptance coverage.
- Safety: No production command-capable route was enabled.
...
- Outcome: Complete (2026-09-03).
- Delivered: Account authentication, signed host registration, recovery, revocation, and deletion flows.
- Verified: Security review findings were resolved; remote commands remained disabled.

**Findings resolved**

- Implemented the previously specified but missing account-deletion state
  machine: recent second factor, seven-day cooling-off, cancellation token,
  24-hour warning, credential erasure, revocation, and permanent username
  tombstoning.
- Bound browser sockets to their authenticated session and revalidated that
  session on every frame; logout, revocation, step-up rotation, and refresh
  rotation now close matching sockets immediately.
- Serialized session changes with browser attachment and revalidated inside
  the attachment lock, closing the rotation-versus-attachment race.
- Rejected queued frames from revoked generations before they can mutate
  pairing-rate state.
- Replaced reversible source-IP hashes in authentication rate keys with keyed
  HMAC network fingerprints.
...
- Outcome: Complete (2026-09-04).
- Delivered: Interoperable host/browser encryption, local pairing, durable device trust, and layered rate limits.
- Verified: Negative and interoperability tests passed; independent review left no critical or high findings.

**Findings resolved**

- Replaced the impossible single human-code design with a bootstrap-key,
  three-packet pairing ceremony and real fingerprint vectors.
- Removed circular host-key trust: the host starts blind, learns the browser
  identity from the encrypted first packet, and the browser binds the offer to
  its authenticated account and host route.
- Added browser-side crypto and persistent non-extractable Ed25519/X25519 keys,
  replay state, outbound sequences, explicit host-key-change approval, and
  durable host trust.
- Added host and relay pairing limits across ceremony, account, device, and
  keyed source-fingerprint scopes.
- Fixed rate-limit window-boundary double counting, premature dedupe expiry,
  redundant storage reads, and extra-field schema evasion.
- Removed the public test relay and bootstrap bearer compatibility path;
  production attachment uses signed-host or authenticated-browser checks.
- Removed the identity index from the live socket path by routing with the
  immutable account ID and rechecking the current username in the account
  object.
- Added host-only lease heartbeats, independent heartbeat deadlines, bounded
  reconnect backoff, terminal policy-close handling, and fresh signed
  challenges on retry.
- Hardened persisted host configuration: canonical UUIDv7/username/URL/key
  epoch validation, loopback-only plaintext URLs, mode 0600 files, mode 0700
  directories, crash-durable replacement, and response binding to the exact
  host ID and public JWKs.
- Kept post-pairing authority limited to `shore.probe`; ADR-0040 commands
  remained disabled throughout this milestone.

**Verification**

- Cross-language vectors match byte-for-byte between Python and TypeScript.
- Cross-process coverage performs real pairing, encrypted probe, host-key
  rotation, explicit replacement approval, and re-pairing without test doubles.
- Negative coverage includes tampering, substitution, epoch mismatch, expiry,
  replay, reordering, confirmation reuse, revocation, injection, rate limits,
  lifecycle races, and reconnect behavior.
- Independent security review completed on 2026-09-04 with no unresolved
  critical or high findings; production deployment remained a separate gate.
...
- Outcome: Complete (2026-09-07); sub-milestones 4.0–4.9 landed.
- Verified: Duplex interop, transport parity, negative authorization, reconnect durability, and documentation gates passed.
- Remaining: Production enablement is a separate explicit deployment decision.
...
- Outcome: Landed.
- Delivered: Host and browser pairing UI with production callers for the complete ceremony.
...
- Outcome: Landed with a narrower implementation than originally proposed.
- Delivered: One reusable replay/snapshot/rollover catch-up function shared by both direct realtime paths.
...
- Outcome: Landed.
- Delivered: An enforced, closed capability registry with default-deny behavior.
- Verified: Review-driven fixes covered schema strictness, identity binding, and denial behavior.
...
- Outcome: Landed together with 4.4 identity plumbing.
- Delivered: Per-device host sessions, authorized dispatch, push delivery, and lifecycle cleanup.
...
- Outcome: Landed in full.
- Delivered: Duplex browser dashboard sessions, cursor persistence, reconnect behavior, and host/browser interop coverage.
...
- Outcome: Landed.
- Verified: Request identity, cursor, acknowledgement, replay, heartbeat, backpressure, and reconnect behavior remain durable.
...
- Outcome: Landed.
- Verified: Direct WebSocket and Shore paths produce matching normalized results for permitted scenarios.
...
- Outcome: Landed.
- Verified: Identity checks precede capability checks and all non-allowlisted operations fail closed without side effects.
...
- Outcome: Landed and acceptance gate closed.
- Delivered: Protocol, architecture, operational, and implementation documentation for Milestone 4.
- Verified: Final independent review found no unresolved critical or high findings.
...
- Fixed a real, pre-existing bug surfaced while building on top of it:
  ...
  (`0.1.5rc9`, currently published on PyPI) - every real release candidate
