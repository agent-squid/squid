...
host creates a random 128-bit `pairing_secret` and 128-bit `ceremony_nonce`,
expires the ceremony after five minutes, and displays both only in a local QR
and a 26-character Crockford-base32 human code (130 encoded bits; leading two
bits are zero). The broker receives only an opaque ceremony ID and ciphertext.
...
The browser encrypts its full public-key bundle and binding with AES-256-GCM
under `pair_key`; the host returns its bundle similarly with a different random
nonce. Both verify the binding, pinned account/host identity, and the other
