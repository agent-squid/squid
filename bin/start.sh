...
# ── tailscale serve ──────────────────────────────────────────────────────────
# Shared with install.sh, which runs the same block right after a fresh
# install so the URLs are visible immediately, not just on first start.
source "$SCRIPT_DIR/tailscale-serve.sh"
