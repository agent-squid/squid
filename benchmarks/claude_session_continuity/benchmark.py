...
class ClaudeGauge:
    ...
    def fetch(self) -> GaugeSnapshot:
        ...
        return GaugeSnapshot(float(utilization), window.get("resets_at"), utc_now())
    ...
    def stable(
        ...
        *,
        ...
        while True:
            current = self.fetch()
            if previous and current.resets_at != previous.resets_at:
    ...
    *,
    ...
    if arm == "persistent":
        with ClaudeStreamProcess(command_factory(False), cwd, env, timeout) as process:
            return [process.ask(prompt, session_id, index) for index, prompt in enumerate(prompts, 1)]

    results: list[PromptResult] = []
    for index, prompt in enumerate(prompts, 1):
        with ClaudeStreamProcess(command_factory(index > 1), cwd, env, timeout) as process:
            results.append(process.ask(prompt, session_id, index))
    return results
...
def sleep_countdown(seconds: float, label: str) -> None:
    if seconds <= 0:
        return
...
def run_benchmark(config: dict[str, Any], output_path: Path, skip_cooldown: bool = False) -> dict[str, Any]:
    ...
    try:
        for position, arm in enumerate(order):
            sleep_countdown(cooldown, f"Cold-cache boundary before {arm}")
            cwd = worktrees.create(arm)
            session_id = str(uuid.uuid4())
            print(f"Starting {arm} arm in {cwd} (session {session_id})", flush=True)
            before = gauge.stable(**gauge_options)
            started_at = utc_now()
            ...
            after = gauge.stable(**gauge_options)
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        ...
        run_benchmark(config, output, skip_cooldown=args.skip_cooldown)
