/**
 * Regression test: a late chat.stats frame for the sender's own completed flow
 * origin must not drop its standalone user prompt. The stats delta re-dirties
 * the completed turn; render()'s completed→completed branch must re-adopt the
 * composer scaffold (route marker + prompt + timestamp) rather than removing it.
 */
...
test('late chat.stats keeps the sender flow origin user prompt', async ({ page }) => {
  ...
  // A real server follows completion with a stats frame. It re-dirties the
  // completed turn via applyRunEvent and triggers a targeted reconcile.
  await emit([
    { v: 1, type: 'chat.stats', event_id: 6, msg_id: 100, run_seq: 2, scope: { topic: 'tst', agent: 'echo' },
